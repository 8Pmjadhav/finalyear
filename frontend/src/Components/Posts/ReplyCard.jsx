...
import { Merge, Pencil, Trash2 ,ThumbsUp } from "lucide-react";
...
export default function ReplyCard({ reply, setRefetch }) {
    ...
    useEffect(() => {
        ...
        const isliked = reply?.likes?.find((ele) => ele.user_id === user.id)
        if (isliked) {
    ...
    async function likeReply() {
        try {
          await client.delete(`/api/reply/likeReply/${reply.id}`,{
            headers:{
              'Authorization' : `Bearer ${localStorage.getItem('accessToken')}`
            }
          })
            ...
            <div className="max-w-xl mx-auto bottom-2  dark:text-white border border-gray-600 shadow-md rounded-md overflow-hidden mb-1">
                ...
                <div className="p-4">
                    ...
                    <div className="text-sm mt-3 flex justify-between">
                        ...
                        <div className="flex items-center">
                            <div>{reply?._count?.likes || 0} &middot; </div>
